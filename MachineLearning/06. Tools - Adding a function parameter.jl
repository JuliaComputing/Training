...
# Now we are ready to draw the function. For each plot, we *fix* a value of the parameter $w$ and draw the resulting function as a function of $x$. We can modify the value of $w$, and plot the new function that comes out:
...
#
# (c) Add as the plot title the value of the loss function for the current value of $w$.
#
# (d) Use the slider to find the value $w^*$ of $w$ for which the loss function reaches its minimum value. What is $w^*$? What is the value of the loss function there, $L(w^*)$?
